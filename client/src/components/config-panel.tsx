...
import { coy } from 'react-syntax-highlighter/dist/esm/styles/prism'; // Choose your preferred theme
...
export function ConfigPanel() {
  const [selectedFile, setSelectedFile] = useState<string>(CONFIG_FILES[0]);
  ...
  const fetchTerraformConfig = async (fileName: string) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => {
    fetchTerraformConfig(selectedFile);
  }, [selectedFile]);
  ...
  return (
    <Card className="bg-card/10 backdrop-filter backdrop-blur-sm relative self-start p-4">
      ...
      {!isLoading && !error && terraformConfig ? (
        <div className="rounded-lg overflow-y-auto overflow-x-auto max-h-[70vh] w-full md:w-40wh bg-gray-900 p-4">
        <SyntaxHighlighter
          language="terraform"
          style={coy}
          showLineNumbers
